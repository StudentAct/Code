...
def run(
                ...
                if len(outputs[i]) > 0:
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            ...
                            label = None if hide_labels else (f'{id} st' if hide_conf else \
                                (f'{id} {conf:.2f}' if hide_class else f'{id} st {conf:.2f}'))
                            # color = colors(c, True)
    ...
    if save_txt or save_vid:
        ...
        LOGGER.info(f"Results saved to {colorstr('bold', save_dir)}{s} with{fps}fps")
    if update:
